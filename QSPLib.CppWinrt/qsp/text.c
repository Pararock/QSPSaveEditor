...
int qspAddText(QSP_CHAR **dest, QSP_CHAR const *val, int destLen, int valLen, QSP_BOOL isCreate)
{
    int ret;
    QSP_CHAR *destPtr;
...
QSP_CHAR *qspGetAddText(QSP_CHAR *dest, QSP_CHAR const *val, int destLen, int valLen)
{
...
QSP_CHAR *qspStrCopy(QSP_CHAR *strDest, QSP_CHAR const *strSource)
{
...
QSP_CHAR *qspStrNCopy(QSP_CHAR *strDest, QSP_CHAR const *strSource, int maxLen)
{
    QSP_CHAR *ret = strDest;
...
int qspStrLen(QSP_CHAR const *str)
{
    QSP_CHAR *bos = str;
    while (*str) ++str;
    return (int)(str - bos);
...
{
    int allocChars, count = 0, bufSize = (5 * 1024 * 1024), delimLen = qspStrLen(delim);
